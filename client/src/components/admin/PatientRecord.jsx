...
// ─────────────────────────────────────────────────────────────────────────────
// Field MUST be defined OUTSIDE PatientRecord.
// If defined inside, React creates a brand-new component type on every render,
// unmounting the <input> on every keystroke — which destroys cursor focus.
// ─────────────────────────────────────────────────────────────────────────────
function Field({ label, value, editKey, readOnly, editMode, form, setForm }) {
  return (
    <div>
...
export default function PatientRecord({ uid, onBack }) {
  ...
  async function save() {
    ...
    try {
      ...
      setEditMode(false);
      setMsg('Patient record updated successfully.');
    } catch (err) {
  ...
  // Shared props passed to every Field so it can read/write form state
  const fieldProps = { editMode, form, setForm };
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <Field label="Full Name"         value={`${patient.first_name} ${patient.middle_name || ''} ${patient.last_name}`.trim()} {...fieldProps} />
          ...
          <Field label="Mobile"            editKey="mobile"            value={`+91 ${patient.mobile}`}      {...fieldProps} />
          <Field label="City"              editKey="city"              value={patient.city}                 {...fieldProps} />
          <Field label="District"          editKey="district"          value={patient.district}             {...fieldProps} />
          <Field label="State"             editKey="state"             value={patient.state}                {...fieldProps} />
          <Field label="Current Address"   editKey="current_address"   value={patient.current_address}      {...fieldProps} />
          <Field label="Permanent Address" editKey="permanent_address"  value={patient.permanent_address}   {...fieldProps} />
